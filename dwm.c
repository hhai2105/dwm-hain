...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		if (occ & 1 << i){
			drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSelTag : SchemeOccTag]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		}else{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeSelBar : SchemeNorm]);
...
{
    Client* fsv = NULL;
    for (Client* cl = selmon->cl->stack; cl && !fsv; cl = cl->snext){
        if (ISVISIBLE (cl, selmon) && cl->isfullscreen)
            fsv = cl;
    }
...
hideunselect(Monitor *m){
	Client *c;
	for (c = m->stack; c && (!ISVISIBLE(c, m) || c->isfloating); c = c->snext);
	for (; c; c = c->snext)
		if (!c->isfloating && ISVISIBLE(c, m))
			toggleshow(c, 0);
}
...
togglescratch(const Arg *arg)
{
	...
	if (found) {
		if(m != selmon){
			...
			c->tags = ISVISIBLE(c, selmon) ? 1 << 20 : selmon->tagset[selmon->seltags];
		}
		focus(NULL);
		arrange(selmon);
